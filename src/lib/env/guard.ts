/**
 * GoldenSoft HR Environment Guard.
 *
 * Adapted from the Platform guard with the same Supabase project pinning and
 * PostgreSQL TLS rules. HR differs in two ways:
 *   - APP_CODE must be HR
 *   - HR never sends Auth invitations, so there is no AUTH_INVITE_MODE contract
 */
...
export function assertSafeEnvironment(
  ...
  const caCertPath = input.caCertPath ?? process.env.SUPABASE_DB_CA_CERT_PATH;
  ...
  if (appCode !== HR_APP_CODE) {
    return {
      ok: false,
      code: "APP_CODE",
      reason: "APP_CODE must be HR for GoldenSoft HR",
    };
  }
  ...
  if (appUrl) {
    const appUrlError = validateAppUrl(appUrl, nodeEnv);
    if (appUrlError) return appUrlError;
  }
  ...
  if (apiRef !== dbRef || apiRef !== directRef) {
    return {
      ...
      code: "REF_MISMATCH",
      reason:
        "Project refs from Supabase URL, DATABASE_URL, and DIRECT_URL must match",
    };
  }
