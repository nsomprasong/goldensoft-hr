/**
 * Cleanup HR login-test rows (EMP-* / TEST_* under แพลูกแพรว).
 * Does not remove Platform Auth — run Platform cleanup for that.
 *
...
async function main() {
  ...
  const { cleanupLoginTestHr } = await import(
  ...
  try {
    const result = await cleanupLoginTestHr(prisma, { dryRun });
    console.log(JSON.stringify(result, null, 2));
    if (!dryRun && result.deleted) {
      console.log(
        "HR rows removed. Next (optional): cd ../goldensoft-platform && npm run seed:login-test:cleanup",
      );
    }
  } finally {
