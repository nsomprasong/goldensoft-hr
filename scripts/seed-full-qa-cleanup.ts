/**
 * Cleanup HR full-QA data.
 *
 *   npm run seed:full-qa:cleanup
 *   npm run seed:full-qa:cleanup -- --dry-run
 */
export {};
...
async function main() {
  ...
  const { cleanupFullQaHr } = await import("../src/lib/seed/full-qa-dataset");
  ...
  if (process.env.NODE_ENV === "production") {
    console.error("full-qa cleanup forbidden in production");
    process.exit(1);
  }
  ...
  try {
    const result = await cleanupFullQaHr(prisma, { dryRun });
