/**
 * HR demo dataset cleanup. Only removes rows carrying the demo prefix.
 * Usage:
 *   npm run seed:hr:demo:cleanup -- --dry-run
 *   npm run seed:hr:demo:cleanup
 */
export {};
...
async function main() {
  ...
  if (!organizationId) {
    console.error(
      "SEED_ORGANIZATION_ID is required for seed:hr:demo:cleanup",
    );
    process.exit(1);
  }
  ...
  try {
    const counts = await cleanupDevelopmentDemo(prisma, organizationId, {
      dryRun,
    });
